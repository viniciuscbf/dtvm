...
w(f"(1, '3f2a1c9e-8b47-4d10-9e2f-6a5d4c3b2a19', 'realtime', 'Ricardo Alves (pessoa física)', 'Ativo', 'Ricardo Nunes', {s(DIAS[-15].isoformat() + ' 10:20:00')}, NULL),")
...
w("-- ---------- portal do cotista: vínculo do token, conta cadastrada e ordens de exemplo ----------")
w("UPDATE tokens_acesso SET cotista_id = 1 WHERE token = '3f2a1c9e-8b47-4d10-9e2f-6a5d4c3b2a19';")
w("UPDATE cotistas SET banco = 'Banco Aurora S.A. (030)', agencia = '0001', conta = '48.732-1',")
w("                    pix_chave = 'ricardo.alves@email.com.br' WHERE id = 1;")
w("INSERT INTO ordens_passivo (fundo_id, cotista_id, tipo, valor, metodo, txid, pagador_doc, status, criado_em) VALUES")
w(f"(1, 1, 'Aplicação', 100000.00, 'Pix', 'A7F3B2C914D85E60A1B2C3D4', '***.172.911-**', 'Recebida', {s(LB.isoformat() + ' 09:12:00')}),")
w(f"(1, 1, 'Aplicação', 25000.00, 'TED', NULL, NULL, 'Aguardando pagamento', {s(LB.isoformat() + ' 10:40:00')}),")
w(f"(1, 1, 'Resgate', 15000.00, NULL, NULL, NULL, 'Solicitado', {s(LB.isoformat() + ' 11:02:00')});")
w()

w("-- ---------- CONTA DE ACESSO do cotista (login e-mail/senha; multi-fundo) ----------")
w("INSERT INTO cotista_contas (id, nome, documento, email, senha_hash, status, criado_em) VALUES")
w(f"(1, 'Ricardo Alves', '***.172.911-**', 'ricardo.alves@email.com.br',")
w(f" '$2y$10$HRgZ/xd2XJ2VtL9hwMuEuOncQUvL4CFNu/WlZNQp2YAKdA1J/rzqq', 'Ativa', {s(DIAS[-15].isoformat() + ' 10:20:00')});")
w("UPDATE cotistas SET conta_id = 1 WHERE id = 1;")
w("UPDATE cotistas SET nome = 'Ricardo Alves', documento = '***.172.911-**', conta_id = 1,")
w("                    banco = 'Banco Aurora S.A. (030)', agencia = '0001', conta = '48.732-1',")
w("                    pix_chave = 'ricardo.alves@email.com.br' WHERE id = 44;")
w("UPDATE fundos SET transparencia = 'realtime' WHERE id = 1;")
w("UPDATE fundos SET transparencia = 'delay_3m' WHERE id = 2;")
w()
