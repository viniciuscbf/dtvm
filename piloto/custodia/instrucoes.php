    ...
    elseif (!empty($_POST['liquidar'])) {
        ...
        if ($ev = $st->fetch()) {
            $pdo->prepare("UPDATE eventos_corporativos SET status='Liquidado', processado_por=?, processado_em=NOW() WHERE id=?")
                ->execute([$u['nome'] . ' (custódia)', $ev['id']]);
            $pdo->prepare("INSERT INTO movimentacoes (fundo_id, data_ref, tipo, descricao, valor) VALUES (?,?,?,?,?)")
                ->execute([$ev['fundo_id'], date('Y-m-d'), 'Provento',
                           "{$ev['tipo']} de {$ev['ativo_codigo']} creditado pelo custodiante", (float)$ev['valor_total']]);
            $pdo->prepare('UPDATE fundos SET caixa_atual = caixa_atual + ? WHERE id = ?')
                ->execute([(float)$ev['valor_total'], $ev['fundo_id']]);
            $msg = "{$ev['tipo']} de {$ev['ativo_codigo']} creditado (" . moeda($ev['valor_total']) . ').';
        }
    }
}
        ...
        <tr>
          ...
          <td>
            <form method="post" class="d-inline">
              <input type="hidden" name="aceitar_boleta" value="<?= (int)$b['id'] ?>">
            ...
            <form method="post" class="d-flex gap-1 mt-1">
              <input type="hidden" name="rejeitar_boleta" value="<?= (int)$b['id'] ?>">
        ...
        <tr class="<?= $ev['status'] === 'Liquidado' ? 'text-muted' : '' ?>">
          ...
          <td class="text-end">
            ...
            <?php elseif ($ev['status'] === 'Provisionado'): ?>
              <form method="post"><input type="hidden" name="liquidar_evento" value="<?= (int)$ev['id'] ?>">
